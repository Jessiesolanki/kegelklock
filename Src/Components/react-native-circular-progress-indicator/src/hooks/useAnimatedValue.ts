...
export default function useAnimatedValue({
  ...
  // console.log(animatedValue,'animatedValueanimatedValueanimatedValueanimatedValue')
...
const [isHalf,setisHalf]= useState(0)
  ...
  var mainvalue = value
  let halfvalue = mainvalue/2

  const progressValue = useDerivedValue(() => {
    let value = progressFormatter(animatedValue.value)
    let reversevalue = mainvalue - value
    if (reverse) {
      return `:${reversevalue}`;
    } else {
