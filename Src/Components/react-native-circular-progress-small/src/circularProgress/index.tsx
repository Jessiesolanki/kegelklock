...
const CircularProgress = forwardRef<ProgressRef, CircularProgressProps>((props, ref) => {
  ...
  // console.log(progressValue,'progressValueprogressValue')
  return (
      ...
      <View
        ...
        {showProgressValue && (
          <View style={styles(styleProps).valueContainerRow}>
            ...
            {/* {time?} */}
            {time ? <Text style={{ fontSize: 36, color: activeStrokeColor,marginTop:"-15%",marginBottom:"-5%" }} >{time}</Text>
              : <ProgressValue
        ...
        {title && title !== '' ? (
          <Text
            ...
            style={[styles(styleProps).title, titleStyle,{color:"black"}]}
            numberOfLines={1}
            allowFontScaling={allowFontScaling}>
            {title}
          </Text>
